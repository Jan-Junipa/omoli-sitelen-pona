...
Window_BattleLog.prototype.makeCustomActionText = function(subject, target, item) {
  ...
  function parseNoStateChange(tname,stat,hl) {
	...
	// ${stat} pi ${tname} li ken ala ${hl}
	let noStateChangeText = `'${stat}㉍${tname}㈧㈘㈂${hl}`; // TARGET NAME - STAT - HIGHER/LOWER
...
if (hpDam != 0) {
  ...
  var hpDamageText = target.name() + '㈧㈖㉈' + hpDam + '㈀';
  if (strongHit) {
...
if (mpDam > 0) {
  //mpDamageText = target.name() + ' lost ' + mpDam + ' JUICE...';
  var mpDamageText = target.name() + '㈧㈖㈓㈂㈉㉪' + mpDam;
  ...
  case 'SAD POEM':  // SAD POEM
    ...
    if(!target._noEffectMessage) {
      ...
      if(target.isStateAffected(12)) {text += target.name() + '㈧㈖㉎㈍㉈';}
	  //else if(target.isStateAffected(11)) {text += target.name() + ' feels DEPRESSED..';}
      else if(target.isStateAffected(11)) {text += target.name() + '㈧㈖㉎㑒㈍㉣';}
	  //else if(target.isStateAffected(10)) {text += target.name() + ' feels SAD.';}
      else if(target.isStateAffected(10)) {text += target.name() + '㈧㈖㉎㈍';}
    }
    //else {text += parseNoEffectEmotion(target.name(), "SADDER!")}
	else {text += parseNoEffectEmotion(target.name(), "㉎㈍")}
    break;
  ...
  case 'TRIP':  // TRIP
	...
	// (user) li kama anpa e (target)
	
	text = user.name() + ' ㈧㈖㈅㈉ ' + target.name() + '!\r\n'; // Textbox too small to write more //TODO change font-size / shorten party members' names
	
	//if(!target._noStateMessage) {text += target.name() + '\'s SPEED fell!\r\n';}
	// tenpo utala pi (target) li kama anpa
	if(!target._noStateMessage) {text +=  '㉫㉱㉍'+ target.name() +'㈧㈖㈅\r\n';}
    //else {text += parseNoStateChange(target.name(), "SPEED", "lower!\r\n")}
	// tenpo utala, kama lili
	else {text += parseNoStateChange(target.name(), "㉫㉱", "㈖㈨\r\n")}
    text += hpDamageText;
    break;
  ...
  case 'RELEASE ENERGY':  // RELEASE ENERGY
	...
	// (user) en jan poka ona li kulupu
	text = user.name() + '㈊㈑㉒㉆㈧㈟\r\n';
    //text += 'use their ultimate attack!';
	// li kepeken utala suli wawa a
	text += '㈧㈙㉱㉣㉵㈀';
    break;
  ...
  case 'LOOK OMORI 1':  // Look at Omori 2
	...
	// jan[open monsuta o lape insa] li sona ala e lukin ona
	text = '㈑㐥㉇㉽㉄㈢㈏㐓㈧㉡㈂㈉㈮㉆';
	// text += user.name() + ' attacks again!\r\n';
	// ni2 la (user) li utala sin
	text += '㐁㈡' + user.name() + '㈧㉱㉝';
    text += hpDamageText;
    break;
  ...
  case 'LOOK KEL 1':  // Look at Kel 1
	...
	// jan[ken ·] li alasa pilin utala e jan[olin pilin utala lili ike]
	text = '㈑㐢㈘㊉㐓㈧㈃㉎㉱㈉㈑㐥㉅㉎㉱㈨㈍㐓';
  ...
  case 'LOOK HERO':  // LOOK AT HERO 1
	...
	// jan[wawa :] li pana e nasin wawa tawa jan[olin pilin utala lili ike] // idk how to translate this, might be able to if i add more lines
	text = '㈑㐢㉵㊊㐓㈧㉌㈉㈿㉵㉩㈑㐥㉅㉎㉱㈨㈍㐓\r\n';
    //if(target.isStateAffected(6)) {text += target.name() + " feels HAPPY!\r\n"}
	// li kama pilin pona
	if(target.isStateAffected(6)) {text += target.name() + "㈧㈖㉎㉔㈀\r\n"}
    //else if(target.isStateAffected(7)) {text += target.name() + " feels ECSTATIC!!\r\n"}
	// li kama pilin pi[pona suli]
	else if(target.isStateAffected(7)) {text += target.name() + "㈧㈖㉎㑒㉔㉣㈀㈀\r\n"}
    //text += user.name() + '\'s DEFENSE rose!!';
	// wawa awen pi (user) li kama wawa mute
	text +=  '㉵㈈㉍' + user.name() + '㈧㈖㉣';
    break;
  ...
  //KEL//
    case 'ANNOY':  // ANNOY
      ...
      if(!target._noEffectMessage) {
		...
		if(target.isStateAffected(14)) {text += target.name() + '㈧㉎㉱㈀';}
        //else if(target.isStateAffected(15)) {text += target.name() + ' feels ENRAGED!!';}
		else if(target.isStateAffected(15)) {text += target.name() + '㈧㉎㑒㉱㉣㈀㈀';}
        //else if(target.isStateAffected(16)) {text += target.name() + ' feels FURIOUS!!!';}
		else if(target.isStateAffected(16)) {text += target.name() + '㈧㉎㑒㉱㈷㈀㈀㈀';}
      }
    ...
    case 'PASS OMORI':  // KEL PASS OMORI
	  ...
	  // jan[open monsuta o lape insa] li lukin ala
	  // sike li kama lon lawa ona a
	  text = '㈑㐥㉇㉽㉄㈢㈏㐓㈧㈮㈂\r\n';
	  text = '㉜㈧㈖㈬㈤㉆㈀\r\n';
      //text += 'OMORI takes 1 damage!';
	  // jan[open monsuta o lape insa] li kama pakala wan
	  text += '㈑㐥㉇㉽㉄㈢㈏㐓㈧㈖㉈㉳';
      break;
    ...
    case 'PASS AUBREY':  // KEL PASS AUBREY
	  ...
	  // jan[olin pilin utala lili ike] li utala wawa e sike a // ... li weka e ona (need more space)
	  text = '㈑㐥㉅㉎㉱㈨㈍㐓㈧㉱㉵㈉㉜㈀';
      text += hpDamageText;
      break;
    ...
    case 'PASS HERO':  // KEL PASS HERO
	  ...
	  // (user) li tawa sin e sike tawa ijo ike a
	  if(target.index() <= unitLowestIndex) {text = user.name() + '㈧㉩㉝㈉㉜㉩㈌㈍㈀\r\n';}
      text += hpDamageText;
      break;
    ...
    case 'MASSAGE':  // MASSAGE
	  ...
	  else {text += "㈘㈧㈖㈂㈬"} // the possibility (skill) didn't come true? idk
      break;
    ...
    case 'CALM DOWN':  // PLAYER CALM DOWN
	  ...
	  // (user) li kama monsuta ala
	  if(item.id !== 1445) {text = user.name() + '㈧㈖㉽㈂\r\n';} // Process if Calm Down it's not broken;
	  //if(Math.abs(hpDam) > 0) {text += user.name() + ' recovers ' + Math.abs(hpDam) + ' HEART!';}
	  // (user) li kama jo e pilin sin (hpdam) a
      if(Math.abs(hpDam) > 0) {text += user.name() + '㈧㈖㈓㈉㉎㉝' + Math.abs(hpDam) + '㈀';}
      break;
  ...
  //FOREST BUNNY?//
    ...
    //BERLY//
      ...
      case 'PRESENT':  // PRESENT
		...
		// (target) li open e pana
		text = target.name() + '㈧㉇㈉㉌\r\n';
        //text += 'It wasn\'t what ' + target.name() + ' wanted...\r\n';
		// (target) li wile ala e ni · · ·
		text += target.name() + '㈧㉷㈂㈉㉁㊉㊉㊉\r\n';
        //if(!target._noEffectMessage){text += target.name() + ' feels ANGRY! ';}
		// li kama pilin utala a
		if(!target._noEffectMessage){text += target.name() + '㈧㈖㉎㉱㈀';}
        else {text += parseNoEffectEmotion(target.name(), "㉎㉱㈀")}
        break;
