...
(function($) {
	...
	function isTextInCurrentLanguage(text) {
		return true; // whoops sitelen pona is not english
		...
		static loadSavefileInfo(savefileId) {
			...
			if (globalInfo && globalInfo[savefileId]) {
				...
				if(!isTextInCurrentLanguage(globalInfo[savefileId].actorData.name)) {
					const lang = LanguageManager.defaultLanguage();
					globalInfo[savefileId].actorData.name = fixName(LanguageManager._data[lang]["text"]["XX_BLUE"]["Omori_Name_Input"]["defaultname"]) // Assuming this is Sunny as all the other characters that could save
	...
	Game_Actor = class extends Game_Actor {
		...
		name() {
			...
			const lang = LanguageManager.defaultLanguage();
			const defaultName = fixName(LanguageManager._data[lang]["text"]["XX_BLUE"]["Omori_Name_Input"]["defaultname"]);
			let name = this._name
